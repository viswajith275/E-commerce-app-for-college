...
from backend.models import Bid, BidBase, BidStatus, BidCreate, Item, ItemStatus, BidUpdate

bid_routes = APIRouter('/bids')

@bid_routes.get('/mybids', response_model=List[BidBase])
...
def Create_Bid(current_user: UserDep, db: SessionDep, bid_data: BidCreate):
    ...
    exist = db.query(Bid).filter(Bid.bider_id == current_user, Bid.item_id == bid_data.item_id, Bid.status == BidStatus.PENDING).first()

    if exist is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="You cant place multiple bids on the same item!")
    
    new_bid = Bid(bider_id=current_user.id, item_id=bid_data.item_id, bid_price=bid_data.bid_price)
